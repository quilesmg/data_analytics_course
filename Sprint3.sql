...
CREATE TABLE IF NOT EXISTS `credit_card` (
    ...
    `pin` VARCHAR(45) NULL,
    `cvv` VARCHAR(45) NULL,
	`expiring_date` VARCHAR(45) NOT NULL,
    PRIMARY KEY (`id`));
...
-- EX 2.
-- Creamos la vista del promedio de compras realizadas por cada compañía
CREATE VIEW listamarketing1 AS
SELECT company_name, phone, country, avg_amount
FROM company
JOIN (
...
ON company.id = avg_transaction.company_id
ORDER BY avg_amount DESC;


-- EX 3.
-- Creamos la vista de las compañías de Alemania
 CREATE VIEW listamarketing AS
...
-- 3) Eliminamos el campo website de la tabla company
ALTER TABLE company
DROP COLUMN website;

-- 4) cambiamos el nombre de la columna email
ALTER TABLE user 
RENAME COLUMN email TO personal_email;

-- 5) cambiamos el tipo de los datos en la tabla credit_card 
-- y creamos el foreign key para cambiar la relacion con la tabla transaction
ALTER TABLE credit_card
...
ADD CONSTRAINT `card_transaction`
  FOREIGN KEY (`id`)
  REFERENCES `transactions`.`transaction` (`credit_card_id`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;

-- 6) adicionamos columna fecha_atual a la tabla credit_card
ALTER TABLE credit_card
ADD fecha_atual DATE;
